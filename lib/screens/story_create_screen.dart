// lib/screens/story_create_screen.dart
// 스토리 작성 화면입니다.
...
class _StoryCreateScreenState extends State<StoryCreateScreen> {
  ...
  Future<void> _createStory() async {
    ...
    try {
      ...
      // Firestore에 저장
      final storyDocRef = await FirebaseFirestore.instance.collection('stories').add(story.toFirestore());
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('스토리 생성 중 오류가 발생했습니다: $e')),
        );
      }
    } finally {
      if (mounted) {
